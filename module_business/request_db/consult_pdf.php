...
if ($_GET['id']) {
    ...
    $query2 = "SELECT *
                ...
                WHERE numero_cotizacion = $n_cotizacion
            ";
    ...
    while ($a = $result2->fetch_assoc()) {
        ...
        $cod_p = $a['id_producto'];
        $query3 = "SELECT codigo_producto 
                FROM producto
                WHERE id_producto = '$cod_p'
            ";
...
//Agregamos la fuente
    $pdf->selectFont('../../assets/library/ezpdf/fonts/Helvetica.afm');
...
//$pdf->ezImage($imagen,$padding = 5,$width = 50,$resize = 'full',$justification = 'center');
    ...
    $pdf->ezsetdy(-10);
    $pdf->ezText(utf8_decode("<b>Numero de Cotizacion: </b>" . $row_cotizacion['numero_cotizacion']), 12, array('justification' => 'left'));
    $pdf->ezsetdy(14);
    $pdf->ezText(utf8_decode("Fecha de cotizacion: " . $row_cotizacion['fecha_cotizacion']), 12, array('left' => 200));
